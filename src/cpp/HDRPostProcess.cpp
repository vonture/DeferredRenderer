...
HDRPostProcess::HDRPostProcess()
	: _timeDelta(0.0f), _adaptationRate(2.0f), _keyValue(0.2f), _luminanceMapPS(NULL),
	  _toneMapPS(NULL)
{
	for (UINT i = 0; i < 2; i++)
	{
...
HRESULT HDRPostProcess::OnD3D11CreateDevice(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc)
{
	...
	// Create enough mip levels so that the smallest is one pixel by taking the log2 of the
	// largest texture dimension
	_mipLevels = (UINT)ceil(log((float)max(pBackBufferSurfaceDesc->Width, pBackBufferSurfaceDesc->Height)) / log(2.0f));
